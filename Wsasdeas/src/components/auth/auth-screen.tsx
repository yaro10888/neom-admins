...
export function AuthScreen() {
  ...
  async function doSignup() {
    ...
    try {
      ...
      // بعض إعدادات Supabase لا تُنشئ جلسة مباشرة عند التسجيل.
      // نحاول الدخول فوراً: ينجح إذا كان الحساب مؤكَّداً تلقائياً، فيمر
      // المستخدم إلى شاشة انتظار التفعيل بلا أي خطوة إضافية.
      if (!data.session) {
        const { error: autoLoginError } = await client.auth.signInWithPassword({
          email: signup.email.trim(),
          password: signup.password,
        });
